...
test("coding plan mode forwards chat completions and returns normalized responses payload", async () => {
  let capturedPath = "";
        ...
        choices: [
          {
            message: {
              role: "assistant",
              content: "hello from coding plan"
            }
          }
        ]
      }));
      ...
      output: [
        {
          ...
          role: "assistant",
          status: "completed",
          content: [{ type: "output_text", text: "hello from coding plan" }]
        }
      ]
    });
  } finally {
